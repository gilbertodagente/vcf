...
my $snap_index_directory = $ARGV[15];
...
if ($origin eq 'UCLA' || $origin eq 'Other'){
...
while (!defined $defined){
	if (-e "$analysis_dir/phase1/merge/$sam_file.sorted.bam"){
		...
		$command = "$bin_dir/snap paired $snap_index_directory $temp_first_pair $temp_second_pair -M -s -so -o $analysis_dir/phase1/merge/$sam_file.bam";
		print "Starting SNAP\n$command\n";
		my @args = ("$command");
		system(@args);
		my $retval = $? >> 8;
	...
	#RGDT=Iso8601Date	Read Group run date Default value: null.
		...
		print "Indexing $analysis_dir/phase1/merge/$sam_file.sorted.bam\n";
		`$bin_dir/samtools index $analysis_dir/phase1/merge/$sam_file.sorted.bam`;	
	}
}
